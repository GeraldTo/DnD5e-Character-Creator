...
export default function Weapons(props) {
    ...
    useEffect(() => {
        axios.get(props.url + '/api/equipment-categories/simple-weapons/')
            .then(response => {
                handleInit(response.data, setSimpleList)
            })

        axios.get(props.url + '/api/equipment-categories/martial-weapons/')
            .then(response => {
    ...
    function weapons(list, type, index) {
        ...
        const weaponList = list.map((e, j) => <Dropdown.Item key={j} eventKey={e.index}>{e.name}</Dropdown.Item>)
                ...
                {totalWeapons.map((current, i) =>
                    <ListGroup.Item key={i}>
                        <Form>
                            ...
                            {current && <ListGroup className={styles.Lists}>
                                <ListGroup.Item>
                                    <h3>{current.name}</h3>
                                </ListGroup.Item>
                                <ListGroup.Item>
                                ...
                                <ListGroup.Item>
                                    ...
                                    />
                            ATK Bonus: +{(totalWeapons[i].prof ? (1 + Math.ceil(props.level / 4)) : 0)
                                        + (current.strMod ? props.totalScore[0].mod : props.totalScore[1].mod)}
                                </ListGroup.Item>
                                <ListGroup.Item>
                                    Damage: {current.damage ? current.damage.damage_dice
                                        + ' ' + current.damage.damage_type.name : 'None'}
                                </ListGroup.Item>
                                <ListGroup.Item>
                                    Properties: {current.properties.map(e => e.name).join(', ')}
                                </ListGroup.Item>
                                <ListGroup.Item>
                                    <Form.Check
                                        type="radio"
